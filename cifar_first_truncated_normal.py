...
with learning rate standard = 0.0001
...
def neural_network(input_data):
    hl1 = {'weights':tf.Variable(tf.truncated_normal([n_bytes,n_nodes_hl1],stddev=0.0001)),
           'biases': tf.Variable(tf.truncated_normal([n_nodes_hl1],stddev=0.0001))}
    
    
    
    hl2 = {'weights':tf.Variable(tf.truncated_normal([n_nodes_hl1,n_nodes_hl2],stddev=0.0001)),
           'biases': tf.Variable(tf.truncated_normal([n_nodes_hl2],stddev=0.0001))}
    
    
    hl3 = {'weights':tf.Variable(tf.truncated_normal([n_nodes_hl2,n_nodes_hl3],stddev=0.0001)),
           'biases': tf.Variable(tf.truncated_normal([n_nodes_hl3],stddev=0.0001))}
    

    hl4 = {'weights':tf.Variable(tf.truncated_normal([n_nodes_hl3,n_nodes_hl4],stddev=0.0001)),
		   'biases': tf.Variable(tf.truncated_normal([n_nodes_hl4],stddev=0.0001))}
    
    
    hl5 = {'weights':tf.Variable(tf.truncated_normal([n_nodes_hl4,n_nodes_hl5],stddev=0.0001)),
		   'biases': tf.Variable(tf.truncated_normal([n_nodes_hl5],stddev=0.0001))}
    
    
    output = {'weights':tf.Variable(tf.truncated_normal([n_nodes_hl5,n_classes],stddev=0.0001)),
           'biases': tf.Variable(tf.truncated_normal([n_classes],stddev=0.0001))}
...
#we extract a random batch from the test set
...
def train_op(data):
    ...
    with tf.Session() as sess:
        ...
        print('Test accuracy: ',accuracy.eval({x:random_x,y:random_y}))
